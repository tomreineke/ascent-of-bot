...
open class MediumRangeMissileLauncher(initializer: Initializer) : RocketLauncher(initializer) {
    init {
        name = "Medium Range Missile Launcher"
        weight = 3.0f
        icon = Images.Minigun2
        initiativeCost = 3
...
internal object MediumRangeMissileLauncherAppearance : ActionButtonStylings({
    defineStyling(
        category = ActionCategory.Attack,
        tool = entityTypeOf<MediumRangeMissileLauncher>()
    ) {
        //FIXME[T] Does minigun adequately represent a missile launcher?
